#!/usr/bin/env python3
"""
This script performs lane detection on a video file or a camera feed.
It uses computer vision techniques to identify lane lines and a pre-trained
...
if cameraFeed:
    ...
    output_file = args.video[:-4].rsplit('/')[-1] + '_Detection.mp4'
    output_file_lane_tracker = args.video[:-4].rsplit('/')[-1] + '_LaneTracker.mp4'
count=0
noOfArrayValues =10
...
# Get the dimensions of the stacked image for the VideoWriter
...
if success:
    ...
    h, w, _ = imgStacked.shape
    video_writer = cv2.VideoWriter(os.path.join(args.output_dir, output_file), cv2.VideoWriter_fourcc(*'mp4v'),
                                   cap.get(cv2.CAP_PROP_FPS), (w, h))
    video_writer_lane = cv2.VideoWriter(os.path.join(args.output_dir, output_file_lane_tracker), cv2.VideoWriter_fourcc(*'mp4v'),
                                      cap.get(cv2.CAP_PROP_FPS), (frameWidth, frameHeight))
...
while True:
    ...
    video_writer.write(imgStacked)
    video_writer_lane.write(imgFinal)
